...
const App = () => {
  ...
  const submitForm = e => {
    ...
    if(
      ...
      telForm.valido === 'true'
        ...
        enviarInfo(nombreForm, emailForm, telForm);
    }else{
  ...
  const enviarInfo = e => {
    ...
    fetch('https://vaa95pwkke.execute-api.us-east-1.amazonaws.com/dev/', requestOptions)
    .then(response => {
      if(response.status === 200){
        setNombre({campo: '', valido: null});
        setEmail({campo: '', valido: null});
        setTel({campo: '', valido: null});
        window.location.href = "https://flic.kr/ps/3YKfpi";
      }
    });
